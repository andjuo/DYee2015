...
class TriggerSelection_t{
  
 public:
  TriggerSelection_t(TriggerConstantSet constantsSet, bool isData, int run, HLTEfficiencyCalcDef hltEffCalc=HLTEffCalc_2011Old):
  ...
  TriggerSelection_t(const TString& constantsSetString, bool isData) :
    _constants(DetermineTriggerSet(constantsSetString)),
    _isData(isData),
    _run(0),
    _hltEffCalcAlgo(HLTEffCalc_2011Old)
  {}

  TriggerSelection_t(const TString& constantsSetString, bool isData, int run):
    _constants(DetermineTriggerSet(constantsSetString)),
    _isData(isData),
    _run(run),
  ...
  TriggerSelection_t(const TriggerSelection_t &ts) :
...
//     }
  ...
  Bool_t matchTriggerObjectBitAnyOrder(ULong_t bit1, ULong_t bit2, UInt_t run) const {
    ULong_t lead=this->getLeadingTriggerObjectBit(run);
    ULong_t trail=this->getTrailingTriggerObjectBit(run);
    Bool_t result = ( ( lead & bit1   &&  trail & bit2 ) ||
		      ( lead & bit2   &&  trail & bit1 ) ) ? true : false;
    return result;
  }
