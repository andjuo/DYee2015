...
class TriggerSelection_t : BaseClass_t {
  
 private:
  TriggerConstantSet _constants;
  bool _isData;
 public:
  TriggerSelection_t(const TString &trigSetString, bool actsOnData) : 
    BaseClass_t("TriggerSelection"),
    _constants(TrigSet_UNDEFINED),
    _isData(actsOnData)
  {
    if (!this->triggerSet(trigSetString)) this->reportError("in constructor");
  }

  TriggerSelection_t(const TriggerSelection_t &ts) :
    BaseClass_t("TriggerSelection"),
    _constants(ts._constants),
    _isData(ts._isData)
  {
  }
  ...
  void actOnData(bool actsOnData) { _isData=actsOnData; }
