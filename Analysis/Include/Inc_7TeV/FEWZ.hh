...
protected:
  ...
  bool fInitialized, fCutZPT100, fRegularized;
public:
  FEWZ_t(bool loadWeights, bool do_cutZPT100=true, bool regularize=false);
  bool isInitialized() const { return fInitialized; }
  bool cutZPT100() const { return fCutZPT100; }
  bool regularized() const { return fRegularized; }

  int getIndex(double gen_ee_mass) const {
    int ibinPreFsr = DYTools::_findMassBin(gen_ee_mass,_nMassBinsFEWZ,_massBinLimitsFEWZ);
    return ibinPreFsr;
  }

  double getMassCenter(int ibin) const {
    return 0.5*(_massBinLimitsFEWZ[ibin]+_massBinLimitsFEWZ[ibin+1]);
  }

  TH2D* getHistoPtr(double gen_ee_mass) const {
    int ibinPreFsr = this->getIndex(gen_ee_mass);
    return weights[ibinPreFsr];
  }

  TH2D* getHistoPtr(int ibinPreFsr) const {
    return weights[ibinPreFsr];
  }
  ...
  Double_t getValue(TH2D **values, Double_t gen_ee_mass, Double_t gen_pt, Double_t gen_rapidity) const {
    ...
    //std::cout << "gen_ee_mass=" << gen_ee_mass << " whistoTitle=" << values[ibinPreFsr]->GetTitle() << "\n";
