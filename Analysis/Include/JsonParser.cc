...
int JsonParser::Initialize(const TString &filename){

  Reset();

  ifstream jsonFile(filename.Data());
  if (!jsonFile) {
    cout << "JsonParser::Error: Unable to open JSON file" << endl;
    return 0;
  }
  ...
  std::vector<int> runNumberDigits;
  runNumberDigits.clear();
  int runNumber = 0;
  int depth = 0;
  std::vector<int> firstLumiDigits;
  firstLumiDigits.clear();
  int firstLumi = 0;
  std::vector<int> secondLumiDigits;
  secondLumiDigits.clear();
  int secondLumi = 0;
  std::vector<int> allRunLumis;
  allRunLumis.clear();
  while ( ! jsonFile.eof() ) {
...
bool JsonParser::HasRunLumi(int run, int lumi) const {
  ...
  std::vector < std::vector < int > >::const_iterator iter_irun;
  std::vector<int>        ::const_iterator iter_ilumi;
...
int JsonParser::AssembleNumber(const std::vector<int> &data) const {
